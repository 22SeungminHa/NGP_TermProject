...
int GamePlay = start, Scheck = 0, score = 0, blockDown = 0, random, PrintLc = 3, starcnt = 0;
...
doubleRECT ballrc;
Block list[43];
int Map[15][25], SurvivalMap[SVMAPCNT][4][9];
bool isSwitchOff;
double remx, remy;
ifstream in;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN: {
		if (GamePlay == start) {
			...
			else if (MouseLC.x >= 1368 && MouseLC.x <= 1448 && MouseLC.y >= 48 && MouseLC.y <= 128) { // 홈버튼
				Scheck = click;
				GamePlay = start;
			}
